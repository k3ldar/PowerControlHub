...
PageRelaySettings::PageRelaySettings(Stream* serialPort,
...
void PageRelaySettings::onEnterPage()
{
...
void PageRelaySettings::begin()
{
    // not used just now
}

void PageRelaySettings::refresh(unsigned long now)
{
    (void)now;
...
void PageRelaySettings::handleTouch(uint8_t compId, uint8_t eventType)
{
    (void)eventType;
...
void PageRelaySettings::loadConfigPage()
{
    Config* config = getConfig();
...
void PageRelaySettings::handleText(const char* text)
{
    if (text == nullptr)
    {
        _externalData[0] = '\0';
...
bool PageRelaySettings::validateExternalData()
{
    Config* config = getConfig();
...
void PageRelaySettings::clearSelection()
{
    for (uint8_t i = 0; i < ConfigRelayCount; i++)
    {
        const char* buttonName = reinterpret_cast<const char*>(pgm_read_ptr(&SettingRelayButtons[i]));
