...
PageSettings::PageSettings(Stream* serialPort,
...
void PageSettings::onEnterPage()
{
    _currentSettingsIndex = SettingBoatName;
    loadConfigPage();
}

void PageSettings::begin()
{
    // not used just now
}

void PageSettings::refresh(unsigned long now)
{
	(void)now;
...
void PageSettings::handleTouch(uint8_t compId, uint8_t eventType)
{
    (void)eventType;
...
void PageSettings::loadConfigPage()
{
    ...
    switch (_currentSettingsIndex)
    {
        case SettingBoatName:
    		loadPage(FPSTR(SettingsBoatName), config->location.name, SettingBoatNameLen, ShowAlphaKeyboard, false);
			break;

		case SettingCallSign:
            loadPage(FPSTR(SettingsCallSign), config->location.callSign, SettingCallSignLen, ShowAlphaKeyboard, true);
            break;

        case SettingMmsiNumber:
            loadPage(FPSTR(SettingsMmsiNumber), config->location.mmsi, SettingMmsiNumberLen, ShowNumericKeyboard, true);
			break;

        case SettingHomePortName:
            loadPage(FPSTR(SettingsHomePort), config->location.homePort, ConfigHomePortLength - 1, ShowAlphaKeyboard, true);
			break;
        ...
        default:
            ConfigManager::save();
            navigateTo(PageIdAbout);
    }
}

void PageSettings::loadPage(const char* settingName, const char* settingValue, uint8_t maxInputLength, uint8_t keyboardType, bool allowPrevious)
{
...
void PageSettings::loadPage(const __FlashStringHelper* settingName, const char* settingValue, uint8_t maxInputLength, uint8_t keyboardType, bool allowPrevious)
{
    sendValue(FPSTR(SettingsKeyboardInputLen), maxInputLength);
    sendValue(FPSTR(SettingsKeyboardNumericOnly), keyboardType);
    sendText(FPSTR(SettingsName), settingName);
...
void PageSettings::handleText(const char* text)
{
    if (text == nullptr)
    {
        _externalData[0] = '\0';
...
bool PageSettings::validateExternalData()
{
    ...
    {
    case SettingBoatName:
        strncpy(config->location.name, _externalData, ConfigMaxNameLength - 1);
        config->location.name[ConfigMaxNameLength - 1] = '\0';
        result = true;

        break;

    case SettingCallSign:
        strncpy(config->location.callSign, _externalData, ConfigCallSignLength - 1);
        config->location.callSign[ConfigCallSignLength - 1] = '\0';
        result = true;
    ...
    case SettingMmsiNumber:
        ...
        strncpy(config->location.mmsi, _externalData, ConfigMmsiLength - 1);
        config->location.mmsi[ConfigMmsiLength - 1] = '\0';
        result = true;
    ...
    case SettingHomePortName:
		...
		strncpy(config->location.homePort, _externalData, ConfigHomePortLength - 1);
        config->location.homePort[ConfigHomePortLength - 1] = '\0';
        result = true;
