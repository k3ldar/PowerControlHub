...
PageFlags::PageFlags(Stream* serialPort,
...
void PageFlags::begin()
{
    updateFlagDisplay();
}

void PageFlags::refresh(unsigned long now)
{
    (void)now;
}

void PageFlags::updateFlagDisplay()
{
...
void PageFlags::handleTouch(uint8_t compId, uint8_t eventType)
{
    (void)eventType;
